...
- 지정한 파일 검사
  ```shell
  npx eslint ./src/pages/board/list.ts
  ```
...
- 지정한 파일을 포맷에 맞춰서 변환
  ```sh
  npx prettier --write ./src/pages/board/list.ts
  ```
...
- js, ts, html 파일을 저장할 때 vscode > OUTPUT > Prettier 에서 Prettier 로그가 출력되지 않으면 Prettier 플러그인이 동작하지 않고 있는 것이므로 다음 설정을 추가
...
- 어떻게 변경되었는지(어떻게는 코드를 보면 되므로) 아니라 무엇을, 왜 변경하는지를 설명
