...
## 3.8 accessToken 만료
- 만료된 accessToken은 더이상 사용할 수 없어서 사용자 인증이 안되므로 재발급 필요

### 3.8.1 accessToken 만료 테스트
...
- 서버에서 401 Unauthorized 상태코드로 응답을 하면 refreshToken을 이용해 accessToken을 재발급하는 로직을 구현해 두면 refreshToken 만료기간 만큼 재 로그인 없이 사용자 인증 가능
...
3. 재발급 받은 accessToken으로 기존 스토리지에 저장해둔 토큰을 갱신하고 401 상태코드를 응답받았던(실패했던) 요청을 재시도
