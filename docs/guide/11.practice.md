...
- 프로젝트 루트에 .github/PULL_REQUEST_TEMPLATE.md 파일 생성
  - 여러개의 PR 템플릿을 만들려면 .github/PULL_REQUEST_TEMPLATE 폴더를 생성하고 하위에 .md 파일 작성
...
- 커밋 메시지인 `feature: PR 테스트` 선택해서 코드 확인 후 Review changes
...
- Merge pull request 선택
  - Create a merge commit
    - PR 브랜치를 그대로 merge하고 새로운 커밋을 만듦
    - 커밋 히스토리 그대로 유지, 여러 커밋을 하나로 합치지 않음
  - Squash and merge
    - PR의 여러 커밋을 하나로 합쳐서 merge
    - 깔끔한 히스토리를 원할 때 사용
  - Rebase and merge
    - PR 브랜치 커밋을 대상 브랜치 가장 위로 재배치(rebase)
    - merge 커밋 없이 커밋 히스토리를 직선으로 유지
